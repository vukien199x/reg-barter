...
class VerifyBarter:
    ...
    REQUIRED = ["email", "passwd", "phone", "first_name", "last_name"]
    ...
    def check_info(self):
        ...
        for field in self.REQUIRED:
            if field not in self.info_:
                errors.append(field)
        if errors:
            self.logger_.debug(f"File info.json wrong format. {', '.join(errors)} not exists")
            return False
    ...
    def verify(self):
        ...
        proxies = None
        if proxy:
            if "user" in proxy and "passwd" in proxy:
                url = f"{proxy['mode']}://{proxy['user']}:{proxy['passwd']}@{proxy['host']}:{proxy['port']}"
            else:
                url = f"{proxy['mode']}://{proxy['host']}:{proxy['port']}"
            proxies = {
                "http": url,
                "https": url
            }
        ...
        self.logger_.debug(f"Res => {res}")
        skip = False
        if res.get("Status", "fail") == "fail":
            self.logger_.debug(f"Sign up fail with => {res.get('Message')}")
            if "mobile number already exist" in res.get("Message"):
                return False
            res = self.client_.login()
            self.logger_.debug(f"Res => {res}")
            skip = True
        ...
        if res.get("Status") == "fail":
            self.logger_.debug(res.get("Message"))
            return False
        redirect_url = res["Data"]["RedirectUrl"]
        token = redirect_url.replace(config.ALCHEMY_URL, "")
        self.logger_.debug(f"Action alchemy with token => {token}")
        ...
        res = veriff_client.waiting_rooms()
        self.logger_.debug(f"Res => {res}")
        ...
        veriff_client.actions(actions)
        res = veriff_client.verifications(session_id, "documents", {
            "country": self.client_.country.upper(),
            "type": "DRIVERS_LICENSE"
        })
        self.logger_.debug(f"Res => {res}")
        veriff_client.actions(["document_selected"])
        res = veriff_client.status(session_id)
        self.logger_.debug(f"Res => {res}")
        actions = [
        ...
        actions = [
            ...
            {"event_type": "video_file_saved",
             "params": {"additional_data": {"duration": int(video_front.duration * 1000),
        ...
        actions = [
            "video-started",
            {"event_type": "video_file_saved", "params": {"duration": int(video_back.duration * 1000),
        ...
        actions = [
            {"event_type": "video_file_saved",
             "params": {"duration": int(face.duration * 1000), "file_length": face_length}},
        ...
        veriff_client.actions(actions)
        # Submitted
        res = veriff_client.status(session_id, "submitted")
        self.logger_.debug(f"Res => {res}")
        actions = [
